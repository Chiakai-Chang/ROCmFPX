...
struct llama_lazy_reader {
    ...
    // load_lazy_reader() never constructs this reader on this platform, so this is never called;
    // stubbed as a no-op (not GGML_ABORT like gather() above) since it is purely an advisory hint.
