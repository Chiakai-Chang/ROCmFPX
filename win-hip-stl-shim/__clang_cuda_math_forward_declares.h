...
// Local shim: recent MSVC STL (GH-4609, VS 17.11+) already declares the
// comparison macros as constexpr builtins, which are implicitly
// host+device in HIP; re-declaring them __device__-only is an error.
#if !defined(_MSVC_STL_VERSION) || _MSVC_STL_UPDATE < 202404L
